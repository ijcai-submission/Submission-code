...
% 0002 %ENFRAME split signal up into (overlapping) frames: one per row. [F,T]=(X,WIN,HOP)
% 0003 %
...
% 0009 %               P=enframe(S,W,HOP,'sdp',FS);        % computer first half of PSD (HOP = frame increment in samples)
% 0010 %
...
% 0017 %               [F,T,WS]=enframe(S,W,1/OV,'fa');      % do STFT: one row per time frame, +ve frequencies only
% 0018 %               ... process frames ...
% 0019 %               X=overlapadd(irfft(F,NW,2),WS,HOP);  % reconstitute the time waveform with scaled window (omit "X=" to plot waveform)
% 0020 %
...
% 0029 %                  'f'  perform a 1-sided dft on each frame (like rfft)
...
% 0034 %                  's'  scale window so that power is preserved: sum(mean(enframe(x,win,hop,'sp'),1))=mean(x.^2)
% 0035 %                  'S'  scale window so that total energy is preserved: sum(sum(enframe(x,win,hop,'Sp')))=sum(x.^2)
% 0036 %                  'd'  make options 's' and 'S' give power/energy per Hz: sum(mean(enframe(x,win,hop,'sp'),1))*fs/length(win)=mean(x.^2)
...
% 0060 %      Version: $Id: enframe.m 9529 2017-02-25 19:08:56Z dmb $
% 0061 %
